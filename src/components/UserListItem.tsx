...
type UserListItemProps = {
  userEntity: LegalEntity;
};

const UserListItem: FC<UserListItemProps> = ({ userEntity }) => {
  const { emailAddresses, fullName, walletAddresses, profileImage, linkedAccounts } = userEntity;
          ...
          <div className="text-md text-gray-500 ">
            ...
            <div className="flex flex-wrap mt-4">
              {linkedAccounts.map((account, i) => {
                return <SocialLinkItem type={account.type} url={account.url} key={i} />;
              })}
            </div>
          </div>
        </>
