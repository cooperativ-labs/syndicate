...
type RealEstateProperty = Database['public']['Tables']['real_estate_property']['Row'];
...
export async function getReProperty(id: string): Promise<RealEstatePropertyWithAddresses | null> {
...
export async function getRealEstateProperties(
  ...
  const { data: properties, error } = await supabase
    .from('real_estate_property')
    .select(
      ['*', 'addresses:address(*)', 'images:real_estate_property_image(*, image:image(*))'].join(
        ', '
      )
    )
