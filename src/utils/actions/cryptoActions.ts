...
import { CryptoAddress, LegalEntity, OfferingSmartContractSet, SmartContract } from '@/types';
...
type CreateSwapContractResult = {
  affectedCount: number;
  records: Pick<OfferingSmartContractSet, 'id' | 'swap_contract_id' | 'offering_id'>[];
};
...
type CreateDistributionContractResult = {
  affectedCount: number;
  records: Pick<OfferingSmartContractSet, 'id' | 'distribution_contract_id' | 'offering_id'>[];
};
