...
export async function createCryptoAddress(
  { address, type, chainId, protocol, ownerId, revalidationPath }: {
    address: string;
...
export async function updateCryptoAddress(
  { id, name, isPublic, revalidationPath }: {
    id: string;
    name?: string | null;
...
export type CreateSmartContractResult = {
  affectedCount: number;
  records: Pick<
    SmartContract,
...
export async function createSmartContract(
  { cryptoAddressId, ownerId, type }: CreateSmartContractParams,
): Promise<CreateSmartContractResult> {
  const supabase = createClient();
  ...
  return {
    affectedCount: typeof count === "number" ? count : (data?.length ?? 0),
    records: (data ?? []) as CreateSmartContractResult["records"],
  };
}
...
export async function addContractPartition(
  { smartContractId, partition, revalidationPath }: {
...
export async function createContractSet(
  { offeringId, shareContractId }: CreateContractSetParams,
): Promise<CreateContractSetResult> {
  const supabase = createClient();
  const { data, error, count } = await supabase
...
export async function updateUnestablishedSmartContract(
  { id, established, revalidationPath }: UpdateUnestablishedSmartContractParams,
