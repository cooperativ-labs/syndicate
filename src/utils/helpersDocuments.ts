...
import { Document, DocumentFormat, DocumentFormatType, DocumentType } from '@/types';
...
export const validateFileSize = (
  ...
  if (fileSizeMB > maxSizeMB) {
    return {
      isValid: false,
      errorMessage: `File size (${fileSizeMB.toFixed(
        1
      )}MB) exceeds the maximum limit of ${maxSizeMB}MB for ${isVideo ? 'videos' : 'images'}.`
    };
  }
...
export const validateFileType = (
  ...
  if (!isImage && !isVideo) {
    return {
      isValid: false,
      isVideo: false,
      errorMessage: `Unsupported file type: ${file.type}. Please upload an image (JPEG, PNG, GIF, WebP) or video (MP4, MOV, AVI, WebM, MKV, M4V).`
    };
  }
